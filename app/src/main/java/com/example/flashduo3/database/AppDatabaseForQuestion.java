...
//    };
    ...
    public static AppDatabaseForQuestion getDatabase(final Context context) {
        if (db == null) {
            db = Room.databaseBuilder(context.getApplicationContext(), AppDatabaseForQuestion.class, "question_database").addCallback(new RoomDatabase.Callback() {
                @Override
                public void onCreate(@NonNull SupportSQLiteDatabase db) {
                    super.onCreate(db);
                    Executors.newSingleThreadExecutor().execute(() -> {
                        List<Word> questions = loadQuestionsFromAsset(context);
                        getDatabase(context).wordDao().insertAll(questions);
                    });
                }
            }).build();
        }
